...
`)}function T(s){const t=L(s),i=new TextEncoder().encode(t),r=j.gzip(i,{level:9}),o=new Blob([r],{type:"application/gzip"}),n=(s.title||"anime").replace(/[^a-zA-Z0-9-_]/g,"-").slice(0,60)+".txt.gz",c=document.createElement("a");c.href=URL.createObjectURL(o),c.download=n,c.click(),URL.revokeObjectURL(c.href)}function U(s){return s?s.includes("myanimelist")?"tv":s.includes("anilist")?"link":s.includes("anidb")?"database":s.includes("kitsu")?"film":s.includes("anime-planet")?"globe":s.includes("animenewsnetwork")||s.includes("ann")?"newspaper":"external-link-alt":"external-link-alt"}function E(s){return s?s.includes("myanimelist")?"MyAnimeList":s.includes("anilist")?"AniList":s.includes("anidb")?"AniDB":s.includes("kitsu")?"Kitsu":s.includes("anime-planet")?"Anime Planet":"View":"View"}function R(s){const t=document.getElementById("anime-root");if(!t)return;const i=s.payload||{},r=i.score_avg??s.score?.arithmeticMean??0,o=i.duration_mins??0,n=i.languages||["Subtitled"],c=s.title||"Unknown",u=s.type||"UNKNOWN",p=s.episodes??"Unknown",S=s.status||"UNKNOWN",v=s.animeSeason||{},f=v.year||"N/A",g=(v.season||"N/A").toString(),w=g.charAt(0).toUpperCase()+g.slice(1).toLowerCase(),h=s.picture||"",x=(s.studios||[]).map(d=>String(d)).join(", ")||"Unknown",A=(s.producers||[]).map(d=>String(d)).join(", ")||"Unknown",m=(s.tags||[]).slice(0,15),y=(s.synonyms||[]).slice(0,8),b=(s.sources||[]).slice(0,6),N=h?`<img src="${a(h)}" alt="">`:'<div style="background:#ddd;width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-size:48px;"><i class="fas fa-film"></i></div>';let l=`
  ...
  `;let e="";s.studios?.length&&(e+=`<div class="ad-section"><div class="ad-section-title"><i class="fas fa-building"></i> Studios</div><p>${a(x)}</p></div>`),s.producers?.length&&(e+=`<div class="ad-section"><div class="ad-section-title"><i class="fas fa-handshake"></i> Producers</div><p>${a(A)}</p></div>`),m.length&&(e+=`<div class="ad-section"><div class="ad-section-title"><i class="fas fa-tags"></i> Genres & Tags</div><div class="ad-tags">${m.map(d=>`<span class="ad-tag">${a(d)}</span>`).join("")}</div></div>`),y.length&&(e+=`<div class="ad-section"><div class="ad-section-title"><i class="fas fa-globe"></i> Other Names</div><p>${a(y.join(", "))}</p></div>`),b.length&&(e+='<div class="ad-section"><div class="ad-section-title"><i class="fas fa-stream"></i> Watch Online</div><div class="ad-links-grid">',b.forEach(d=>{e+=`<a href="${a(d)}" target="_blank" rel="noopener" class="ad-link-card"><i class="fas fa-${U(d)}"></i><div><h4>${a(E(d))}</h4><p>Watch now</p></div></a>`}),e+="</div></div>"),e+=`
